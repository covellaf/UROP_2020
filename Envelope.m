...
%for j=1:length(b)
 %non so perchè ma con il for loop ho risultati strani, in principio dovrebbe funzionare  
   % n = b(j)^2 * ((2*mi_0.*t).^(-1));
    n4 = 3^4 * ((2*mi_0.*t).^(-1));
    n3 = 3^2 * ((2*mi_0.*t).^(-1));
    n2 = 2^2 * ((2*mi_0.*t).^(-1));
    ...
    plot( (t*6.242*10^(15)),n4,'r','Linewidth',1);
    hold on
    plot( (t*6.242*10^(15)),n3,'b','Linewidth',1);
    hold on
    plot( (t*6.242*10^(15)),n2,'k','Linewidth',1);
    hold on
    plot( (t*6.242*10^(15)),n1,'c','Linewidth',1);
    ...
    tau4 = (12./n4) .* (Tev.*10^3)/(4*10^6) .* (sigma_v2).^(-1);
    tau3 = (12./n3) .* (Tev.*10^3)/(4*10^6) .* (sigma_v2).^(-1);
    tau2 = (12./n2) .* (Tev.*10^3)/(4*10^6) .* (sigma_v2).^(-1);
    tau1 = (12./n1) .* (Tev.*10^3)/(4*10^6) .* (sigma_v2).^(-1);
    figure(4)
    plot(Tev,tau4,'r','Linewidth',1)
    hold on
    plot(Tev,tau3,'b','Linewidth',1)
    hold on
    plot(Tev,tau2,'k','Linewidth',1)
    hold on
    plot(Tev,tau1,'c','Linewidth',1)
    hold on
    xlabel('temperature $(keV)$','Interpreter','Latex','Fontsize',18);
